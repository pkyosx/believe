...
class Almost(BelieveBase):
    def __init__(self, ts: Union[int, float], ts_range: Union[int, float] = USE_DEFAULT):
...
class AnyInt(BelieveBase):
    def __init__(self, min_value: int = NO_CHECK, max_value: int = NO_CHECK):
    ...
    def validate(self, rhs: int, e_path: str = ""):
        ...
        if self.__min_value != NO_CHECK and self.__min_value > rhs:
            self.raise_validate_error(rhs, e_path=e_path, e_msg=f'value_too_small: {rhs} < {self.__min_value}')
        if self.__max_value != NO_CHECK and self.__max_value < rhs:
            self.raise_validate_error(rhs, e_path=e_path, e_msg=f'value_too_large: {rhs} > {self.__max_value}')


class AnyFloat(BelieveBase):
    def __init__(self, min_value: float = NO_CHECK, max_value: float = NO_CHECK):
